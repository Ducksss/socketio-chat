...
function draw_line(callback) {
  const msgHTML = `<div class="unread-divider" id="unread">Unread Messages</div>`;
...
function appendMessage(name, img, side, text, id, datetime = null) {
  ...
  // console.log(datetime);
  if (side == "right") {
    // In our new CSS, edit_delete are small SVG icons inside the msg-info-time absolute container.
    edit_delete = `
      <img src="images/bin.png" width="12" height="12" border="0" onclick="deleteMessage(${id})" title="Delete" style="margin-right:4px;" />
      <img src="images/edit.png" width="12" height="12" border="0" onclick="editMessage(${id})" title="Edit" style="margin-right:2px;" />
    `;
  } else {
    edit_delete = ``;
  }
  
  // WhatsApp only shows times for received messages inside the bubble bottom right.
  // Our CSS forces msg-info-time to the bottom right of msg-bubble.
  const msgHTML = `
